...
# esd most.sh v0.1.0 (2021-03-10 by MIBonk)
...
if [ -e /net/rcc/mnt/efs-persist/SWDL/skipMostPopup.txt ];then
	rm -rf /net/rcc/mnt/efs-persist/SWDL/skipMostPopup.txt
	echo "skipMostPopup.txt deleted from unit"
	echo "MOST needs to be connected for update"
else
	touch /net/rcc/mnt/efs-persist/SWDL/skipMostPopup.txt
	echo "skipMostPopup.txt set on unit"
	echo "MOST not needed for update"
fi
